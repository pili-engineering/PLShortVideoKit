...
@property (nonatomic, strong) NSIndexPath *iItemIndexPath;
...
- (NSString *)videoInfoWithAVAsset:(AVAsset *)asset {
    NSString *videoInfo = @"May be iCloud file！";
    ...
    if (asset.mediaType == PHAssetMediaTypeVideo) {
        ...
        [self.imageManager requestAVAssetForVideo:asset options:options resultHandler:^(AVAsset * _Nullable asset, AVAudioMix * _Nullable audioMix, NSDictionary * _Nullable info) {
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if (cell.tag == indexPath.item) {
                    cell.infoString = [self videoInfoWithAVAsset:asset];
                }
            });
        }];
    } else {
        cell.videoIndicatorView.hidden = YES;
        cell.infoString = [NSString stringWithFormat:@"%lux%lu",(unsigned long)asset.pixelWidth, (unsigned long)asset.pixelHeight];
...
- (void)updateCollectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    QBImagePickerController *imagePickerController = self.imagePickerController;
    NSMutableOrderedSet *selectedAssets = imagePickerController.selectedAssets;
    self.iItemIndexPath = indexPath;
    PHAsset *asset = self.fetchResult[indexPath.item];
    
    QBAssetCell *cell = (QBAssetCell *)[collectionView cellForItemAtIndexPath:indexPath];
    if (cell.isICloudAsset) {
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"提示" message:@"当前视频在 iCloud 上，需要确认下载后才能编辑！" preferredStyle:(UIAlertControllerStyleAlert)];
        UIAlertAction *sureAction = [UIAlertAction actionWithTitle:@"确认" style:(UIAlertActionStyleDefault) handler:^(UIAlertAction * _Nonnull action) {
            [self performSelector:@selector(allowToDownload) withObject:self afterDelay:1.0];
        }];
        ...
        [self updateCollectionView:collectionView didSelectItemAtIndexPath:indexPath];
    }
...
- (void)allowToDownload {
    ...
    PHAsset *asset = self.fetchResult[self.iItemIndexPath.item];
    [self movieURL:asset];

    QBAssetCell *cell = (QBAssetCell *)[self.collectionView cellForItemAtIndexPath:self.iItemIndexPath];
    cell.isICloudAsset = NO;

    [self.view hideLoadingHUD];
    [self updateCollectionView:self.collectionView didSelectItemAtIndexPath:self.iItemIndexPath];
}

// 允许 iCloud 下载 networkAccessAllowed
