...
dispatch_queue_attr_t lsqGPUImageDefaultQueueAttribute(void);
void lsqRunOnMainQueueWithoutDeadlocking(void (^block)(void));
void lsqRunSynchronouslyOnVideoProcessingQueue(void (^block)(void));
void lsqRunAsynchronouslyOnVideoProcessingQueue(void (^block)(void));
void lsqRunSynchronouslyOnContextQueue(SLGPUImageContext *context, void (^block)(void));
void lsqRunAsynchronouslyOnContextQueue(SLGPUImageContext *context, void (^block)(void));
void lsqReportAvailableMemoryForGPUImage(NSString *tag);

@class SLGPUImageMovieWriter;
...
/** SLGPUImage's base source object
 ...
 - SLGPUImageVideoCamera (for live video from an iOS camera) 
 - SLGPUImageStillCamera.h (for taking photos with the camera)
 - SLGPUImagePicture (for still images)
 - SLGPUImageMovie (for movies)
 ...
 */
@interface SLGPUImageOutput : NSObject
{
    SLGPUImageFramebuffer *outputFramebuffer;
...
@property(readwrite, nonatomic, retain) SLGPUImageMovieWriter *audioEncodingTarget;
@property(readwrite, nonatomic, unsafe_unretained) id<SLGPUImageInput> targetToIgnoreForUpdates;
...
- (SLGPUImageFramebuffer *)framebufferForOutput;
