...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch {
    ...
    // 过滤掉 EffectsView 的 touch 事件
    if ([classArray containsObject:NSStringFromClass(EffectsView.class)]) {
        return NO;
    }
    
    // 过滤掉 FilterPanelView 的 touch 事件
    if ([classArray containsObject:NSStringFromClass(FilterPanelView.class)]) {
        return NO;
    }
...
- (void)setupEditToolboxView {
    ...
    // TuSDK mark - 特效按钮
    button = [self toolBoxButtonWithSelector:@selector(clickTuSDKEffectsButton:)
                                      startX:button.frame.origin.x + button.frame.size.width + 20
                                       title:@"涂图特效"];
...
// 特效按钮点击事件
- (void)clickTuSDKEffectsButton:(UIButton *)btn {
...
- (void)showTuSDKEffectsView {
    ...
    if (!self.tuSDKEffectsView) {
        ...
        // 场景特效视图
        CGFloat height = 200;
        self.tuSDKEffectsView = [[EffectsView alloc]initWithFrame:CGRectMake(0, PLS_SCREEN_HEIGHT - PLS_EditToolboxView_HEIGHT - height - [self bottomFixSpace], PLS_SCREEN_WIDTH, height)];
        self.tuSDKEffectsView.backgroundColor =  PLS_RGBCOLOR(25, 24, 36);
        ...
        revocationButton.frame = CGRectMake(self.tuSDKEffectsView.lsqGetSizeWidth - 40, 30, 30, 30);
    ...
    if (!self.tuSDKEffectsView.superview) {
...
- (void) showTuSDKFiterView {
    ...
    if (!_tuSDKFilterView) {
        ...
        // 滤镜视图
        _tuSDKFilterView = [[FilterPanelView alloc] initWithFrame:CGRectMake(0, PLS_SCREEN_HEIGHT - PLS_EditToolboxView_HEIGHT - filterPanelHeight - [self bottomFixSpace], size.width, filterPanelHeight)];
    ...
    if (!self.tuSDKFilterView.superview) {
        [self.view addSubview:self.tuSDKFilterView];
    }
    self.tuSDKFilterView.hidden = NO;
}
...
- (void)hideTuSDKFilterView {
    self.tuSDKFilterView.hidden = YES;
}
