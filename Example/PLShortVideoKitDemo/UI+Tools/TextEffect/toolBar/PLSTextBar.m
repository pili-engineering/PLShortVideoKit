...
@interface PLSTextBar () <UITextViewDelegate, PLSColorPickerDelegate>

@property (nonatomic, weak) UITextView *textView;

@property (nonatomic, weak) PLSColorPicker *colorSlider;
@property (nonatomic, weak) UIView *keyboardBar;

@end

@implementation PLSTextBar
...
- (void)setShowText:(PLSText *)showText
{
    _showText = showText;
    [self.textView setText:showText.text];
...
- (void)configKeyBoardBar
{
    ...
    PLSColorPicker *_colorSlider = [[PLSColorPicker alloc] initWithFrame:CGRectMake((CGRectGetWidth(keyboardBar.frame)-sliderWidth)/2, (CGRectGetHeight(keyboardBar.frame)-sliderHeight)/2, sliderWidth, sliderHeight) colors:kSliderColors];
...
{
    if ([self.delegate respondsToSelector:@selector(textBarControllerDidCancel:)]) {
        [self.delegate textBarControllerDidCancel:self];
    }
}

- (void)finishButtonClick
{
    if ([self.delegate respondsToSelector:@selector(textBarController:didFinishText:)]) {
        PLSText *text = nil;
        if (self.textView.text.length) {            
            text = [PLSText new];
            text.text = self.textView.text;
            text.textColor = self.textView.textColor;
            CGFloat fontSize = 50.f;
            UIFont *font = [UIFont systemFontOfSize:fontSize];
            text.font = font;
        }
        [self.delegate textBarController:self didFinishText:text];
    }
}
...
- (void)keyboardWillShow:(NSNotification *)notification
{
    ...
    [UIView animateWithDuration:duration animations:^{
        self.keyboardBar.y = self.height-CGRectGetHeight(keyboardRect)-CGRectGetHeight(self.keyboardBar.frame);
        self.textView.height = self.height-self.keyboardBar.y;
    }];
}
