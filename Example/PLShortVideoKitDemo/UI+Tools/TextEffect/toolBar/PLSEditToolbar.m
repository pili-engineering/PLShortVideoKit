...
@interface PLSEditToolbar () <PLSColorPickerDelegate>
...
/** 绘画拾色器 */
@property (nonatomic, weak) PLSColorPicker *draw_colorSlider;

@property (nonatomic, assign) PLSEditToolbarType type;
@property (nonatomic, strong) NSArray *mainImageNormals;
@property (nonatomic, strong) NSArray *mainImageHighlighted;

@end

@implementation PLSEditToolbar
...
- (void)mainBar
{
    ...
    if (self.type&PLSEditToolbarType_draw) {
        [_imageIndexs addObject:@0];
        [_selectIndexs addObject:@(PLSEditToolbarType_draw)];
        buttonCount ++;
    }
    if (self.type&PLSEditToolbarType_sticker) {
        [_imageIndexs addObject:@1];
        [_selectIndexs addObject:@(PLSEditToolbarType_sticker)];
        buttonCount ++;
    }
    if (self.type&PLSEditToolbarType_text) {
        [_imageIndexs addObject:@2];
        [_selectIndexs addObject:@(PLSEditToolbarType_text)];
        buttonCount ++;
    }
...
- (void)drawMenu
{
    if (_edit_drawMenu == nil) {
        ...
        UIButton *edit_drawMenu_revoke = [self revokeButtonWithType:PLSEditToolbarType_draw];
...
//        self.edit_drawMenu_color = colorView;
        ...
        PLSColorPicker *_colorSlider = [[PLSColorPicker alloc] initWithFrame:CGRectMake(CGRectGetMaxX(self.edit_drawMenu_color.frame) + margin + (surplusWidth - sliderWidth) / 2, (CGRectGetHeight(edit_drawMenu.frame)-sliderHeight)/2, sliderWidth, sliderHeight) colors:kSliderColors];
...
- (void)edit_toolBar_buttonClick:(UIButton *)button
{
    switch (button.tag) {
        case PLSEditToolbarType_draw:
        {
            ...
            if (button.isSelected == NO) {
                if ([self.delegate respondsToSelector:@selector(editToolbar:canRevokeAtIndex:)]) {
                    BOOL canRevoke = [self.delegate editToolbar:self canRevokeAtIndex:button.tag];
    ...
    if ([self.delegate respondsToSelector:@selector(editToolbar:mainDidSelectAtIndex:)]) {
        [self.delegate editToolbar:self mainDidSelectAtIndex:button.tag];
    }
}
...
{
    if ([self.delegate respondsToSelector:@selector(editToolbar:subDidRevokeAtIndex:)]) {
        [self.delegate editToolbar:self subDidRevokeAtIndex:button.tag];
    }
    if ([self.delegate respondsToSelector:@selector(editToolbar:canRevokeAtIndex:)]) {
        BOOL canRevoke = [self.delegate editToolbar:self canRevokeAtIndex:button.tag];
...
#pragma mark - PLSColorPickerDelegate
- (void)colorPicker:(PLSColorPicker *)colorPicker didSelectColor:(UIColor *)color {
    self.edit_drawMenu_color.backgroundColor = color;
    if ([self.delegate respondsToSelector:@selector(editToolbar:drawColorDidChange:)]) {
        [self.delegate editToolbar:self drawColorDidChange:color];
    }
}
